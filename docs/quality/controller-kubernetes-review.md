...
| CTRL-A03 | P1 | HPA is a declared ownership type and guardrail, but discovery neither queries HPAs nor emits an HPA ownership signal. KEDA is absent from the domain. A workload controlled by an autoscaler can pass the ownership guardrail. | `domain.OwnershipHPA`, `DetectOwnership`, `Discoverer` |
