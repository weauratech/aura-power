...
[[ "$(jq -r '.spec.template.spec.containers[] | select(.name == "controller") | .readinessProbe.httpGet.path' <<<"$controller_json")" == "/readyz" ]] || { echo "refusing mutation: aggregate controller capability probe is absent" >&2; exit 3; }
