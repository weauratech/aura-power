...
### Added
- Secure self-service password rotation in the API, CLI, and web panel.
- A tested Helm transition from the managed auth Secret to `existingSecret`.

### Security
- Access and refresh tokens now have distinct purposes and session versions. Password and role changes, user deletion, and credential rotation revoke existing sessions.
- SQLite foreign keys are enabled on every connection; orphaned approval history fails closed, and the final administrator cannot be removed or demoted.
